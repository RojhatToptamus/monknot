...
struct MarkprevWorkspaceWindowRequest: Codable, Hashable {
...
final class WorkspaceWindowRequestCenter {
    ...
    private var openWindow: ((MarkprevWorkspaceWindowRequest) -> Void)?
    private var pendingRequests: [MarkprevWorkspaceWindowRequest] = []

    func installOpenWindowAction(_ action: @escaping (MarkprevWorkspaceWindowRequest) -> Void) {
        openWindow = action
    ...
    func openWorkspaceWindow(at url: URL) {
        let request = MarkprevWorkspaceWindowRequest(workspaceURL: url)
