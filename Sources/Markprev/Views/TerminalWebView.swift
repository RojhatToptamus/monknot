...
struct TerminalWebView: NSViewRepresentable {
    ...
    private static func html(
        ...
        <body>
          ...
          window.markprevWrite = function(data) {
            writeAndFollow(data);
          };
          window.markprevReset = function() {
            term.reset();
          };
          window.markprevFocus = function() {
            term.focus();
          };
          window.markprevTheme = function(options) {
    ...
    final class Coordinator: NSObject, WKScriptMessageHandler, WKNavigationDelegate {
        ...
        func updateTheme(
            ...
            evaluate("window.markprevTheme && window.markprevTheme(\(json));", in: webView)
        }

        func focus() {
            guard isLoaded, let webView else { return }
            evaluate("window.markprevFocus && window.markprevFocus();", in: webView)
        }

        private func write(_ text: String, in webView: WKWebView) {
            guard !text.isEmpty, let json = Self.jsonLiteral(text) else { return }
            evaluate("window.markprevWrite && window.markprevWrite(\(json));", in: webView)
        }
