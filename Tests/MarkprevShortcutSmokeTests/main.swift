...
private func context(
    ...
    isBusy: Bool = false
) -> MarkprevKeyboardShortcutContext {
    MarkprevKeyboardShortcutContext(
...
private func action(
    _ key: String,
    _ modifiers: MarkprevKeyboardShortcutModifiers,
    keyCode: UInt16? = nil,
    context: MarkprevKeyboardShortcutContext = context()
) -> MarkprevKeyboardShortcutAction? {
    MarkprevKeyboardShortcutRouter.action(
        for: MarkprevKeyboardShortcutEvent(key: key, modifiers: modifiers, keyCode: keyCode),
        context: context
    )
}
...
expect(
    action(
        "",
        [],
        keyCode: MarkprevKeyboardShortcutRouter.escapeKeyCode,
...
if failures.isEmpty {
    print("MarkprevShortcutSmokeTests passed")
} else {
    for failure in failures {
        print("FAIL: \(failure)")
    }
    fatalError("MarkprevShortcutSmokeTests failed with \(failures.count) failure(s)")
}
