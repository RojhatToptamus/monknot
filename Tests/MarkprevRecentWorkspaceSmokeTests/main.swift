...
@main
struct MarkprevRecentWorkspaceSmokeTests {
    static func main() throws {
        let suiteName = "MarkprevRecentWorkspaceSmokeTests.\(UUID().uuidString)"
        ...
        let first = URL(fileURLWithPath: "/tmp/Markprev/First", isDirectory: true)
        let second = URL(fileURLWithPath: "/tmp/Markprev/Second", isDirectory: true)
        let third = URL(fileURLWithPath: "/tmp/Markprev/Third", isDirectory: true)
        ...
        print("Markprev recent workspace smoke tests passed")
    }
}
