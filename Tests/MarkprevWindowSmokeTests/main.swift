...
@main
struct MarkprevWindowSmokeTests {
    @MainActor
    static func main() {
        ...
        let requestURL = URL(fileURLWithPath: "/tmp/Markprev/Workspace", isDirectory: true)
        let request = MarkprevWorkspaceWindowRequest(workspaceURL: requestURL)
        ...
        print("Markprev window smoke tests passed")
    }
}
